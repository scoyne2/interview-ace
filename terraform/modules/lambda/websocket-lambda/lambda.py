...
def handle_connect(user_id, table, history_table, connection_id):
    """
    Handles new connections by adding the connection ID and user name to the
    DynamoDB table.

    :param user_id: The name of the user that started the connection.
    :param table: The DynamoDB connection table.
    :param history_table: The DynamoDB connection history table.
    ...
    try:
        ...
        now = datetime.now()
        history_table.put_item(Item={"connection_id": connection_id, "user_id": user_id, "connected_at": str(now)})
        logger.info("Added connection %s for user %s.", connection_id, user_id)
    except ClientError:
        logger.exception(
            "Couldn't add connection %s for user %s.", connection_id, user_id
        )
        status_code = 503
    return status_code
...
def handle_message(table, connection_id, event_body, apig_management_client):
    ...
    try:
        ...
        logger.info("Got user name %s.", user_id)
    except ClientError:
        logger.exception("Couldn't find user name. Using %s.", user_id)
    ...
    message = f"{user_id}: {event_body['msg']}".encode("utf-8")
...
def lambda_handler(event, context):
    ...
    history_table_name = os.environ["DYNAMODB_HISTORY_TABLE_NAME"]
    ...
    if route_key == "$connect":
        ...
        response["statusCode"] = handle_connect(user_id, table, history_table, connection_id)
